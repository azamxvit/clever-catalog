// app/[locale]/laminat/page.tsx
import type { Metadata } from "next";
import { getTranslations } from "next-intl/server";
import { CatalogLayout } from "@/components/widgets/CatalogLayout";

export async function generateMetadata(): Promise<Metadata> {
  const t = await getTranslations("catalog.laminat.meta");
  return { title: t("title"), description: t("description") };
}

export default async function LaminatPage() {
  const t = await getTranslations("catalog.laminat");
  ...
  return (
    <div className="pt-20 pb-16 bg-[#f8f5f0] min-h-screen">
      <div className="max-w-[1280px] mx-auto px-5 sm:px-8 lg:px-12">
        ...
        <CatalogLayout defaultCategory="laminate" />
